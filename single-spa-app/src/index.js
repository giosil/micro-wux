...
s instanceof HTMLElement&&(this.root=s),this.root||(this.root=this.id?document.getElementById(this.id):this.context)}else this.internal&&this.internal.mount(this.context),this.root||(this.root=this.id?document.getElementById(this.id):this.context)}if(this._visible||(this.internal?this.internal.visible=!1:this.root.style.display="none"),this._enabled||(this.internal?this.internal.enabled=!1:this.root.setAttribute("disabled","")),this.debug&&console.log("["+c(this)+"] componentDidMount ctx="+c(n)+" root="+c(this.root)),this.componentDidMount(),this.root)for(var l in this.handlers)if(l&&"_"!=l.charAt(0)&&"mount"!=l&&"unmount"!=l&&"statechange"!=l&&"propschange"!=l)for(var a=0,u=this.handlers[l];a<u.length;a++){var h=u[a]
this.root.addEventListener(l,h)}if(wuxRegister(this.root,this),this.mounted=!0,this.id&&(!this.internal||this.internal.id!=this.id)){if(t.registry.indexOf(this.id)>=0){var p=t.getComponent(this.id)
