...
var x=this.line
n&&n.length>b&&((x=n[b])||(x=this.line)),r.beginPath(),r.lineWidth=2,r.strokeStyle=x,r.moveTo(this.offx,i.height-s-v[0]*m)
for(var w=1;w<e.length;w++){var S=this.offx+w*p,_=i.height-s-v[w]*m
r.lineTo(S,_)}r.stroke()}r.beginPath(),r.lineWidth=1,r.strokeStyle=this.axis,r.moveTo(this.offx,this.offy),r.lineTo(this.offx,i.height-s),r.lineTo(i.width-a,i.height-s),r.stroke(),r.beginPath(),r.setLineDash([4,8]),r.lineWidth=1,r.strokeStyle=this.grid
for(var w=1;w<e.length;w++){var S=this.offx+w*p
r.moveTo(S,this.offy),r.lineTo(S,i.height-s)}r.moveTo(this.offx,this.offy),r.lineTo(i.width-a,this.offy)
for(var C=0,O=y;C<O.length;C++){var D=O[C]
r.moveTo(this.offx,i.height-s-D*m),r.lineTo(i.width-a,i.height-s-D*m)}r.stroke(),r.fillStyle=this.axis,r.font=this.fontSize+"px "+this.fontName,r.fillText("0",0,i.height-s)
for(var M=0,W=y;M<W.length;M++){var D=W[M]
r.fillText(""+D,0,i.height-s-D*m)}if(r.fillText(""+g,0,this.offy),h)for(var w=0;w<o.length;w++){var S=this.offx+w*p
r.save(),r.translate(S-this.fontSize,i.height),r.rotate(-Math.PI/3),r.fillStyle=this.axis,r.fillText(o[w],0,0),r.restore()}}}}}},e}(t.WComponent)
